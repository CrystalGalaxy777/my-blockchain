...
const argv = process.argv.slice(2);                                     // EN: CLI args list / DE: CLI-Argumente / RU: Аргументы CLI. Это 2 строки из командной строки в Terminal VS Code после того когда мы запускаем скрипт. Они нам не нужны
...
const timestampStable = hasFlag('--timestampStable') || hasFlag('-t');  // EN: Fix timestamp / DE: Timestamp fixieren / RU: Использовать фиксированный timestamp (метку времени) в блоке, а не брать новое Date.now() каждый раз
